...
var world : Node = null
var debug_id : String = "client"

func _ready():
	local_id = 0
	self.connect("client_log", self, "_on_client_log")
	self.connect("network_log", self, "_on_network_log")
	self.connect("player_scene", self, "player_scene")
	self.connect("player_id", self, "_on_player_id")
	
	get_tree().connect("connected_to_server", self, "_on_net_server_connected")
	get_tree().connect("server_disconnected", self, "_on_net_server_disconnected")
	get_tree().connect("connection_failed", self, "_on_net_connection_fail")
	get_tree().connect("network_peer_connected", self, "_on_net_client_connected")
	get_tree().connect("network_peer_disconnected", self, "_on_net_client_disconnected")
	get_tree().connect("server_up", self, "_on_net_server_up")
...
func client_server_connect(host : String, port : int = DEFAULT_PORT):
	...
	emit_signal("client_log", "network id %s" % client.connection.get_unique_id())
...
func change_scene(scene : String) -> void:
	...
	if not scene in scenes:
		...
		emit_signal("client_log", "No such scene %s" % scene)
		return
	
	emit_signal("client_log", "change_scene to %s" % scene)
	var error = get_tree().change_scene(scenes[scene].path)
	if error == 0 :
		emit_signal("client_log", "changing scene okay(%s)" % error)
		scenes.loaded = scene
		emit_signal("scene_change")
		emit_signal("scene_change_name", scene)
	else:
		emit_signal("client_log", "error changing scene %s" % error)
...
func player_register(pdata : Dictionary, localplayer : bool = false, opt_id : String = "avatar") -> void:
	...
	if localplayer:
		...
		emit_signal("client_log", "player data should have id or be a local")
		return
	
	emit_signal("client_log", "registered player(%s): %s" % [id, pdata])
	var player = {}
	player["data"] = pdata
...
remote func unregister_client(id : int) -> void:
	emit_signal("client_log", "unregister client (%s)" % id)
	if players.has(id):
	...
	if players[id].data.has(prop):
		...
		match prop:
			...
			_:
				error = true
	if error:
		emit_signal("client_log", "error: player data, no property(%s)" % prop)
	return result
...
func player_remap_id(old_id : int, new_id : int) -> void:
	if players.has(old_id):
		...
		player["id"] = new_id
		emit_signal("client_log", "remap player old_id(%s), new_id(%s)" % [old_id, new_id])
		if player.has("path"):
			var node = player.obj
			node.name = "%s" % new_id
			var world = get_tree().current_scene
			emit_signal("client_log", "remap player, old path %s to %s" % [player.path, world.get_path_to(node)])
...
func create_player(id : int) -> void:
	...
	if players[id].has("world") and players[id]["world"] == str(world):
		emit_signal("client_log", "player(%s) already added, %s" % [id, players[id]])
		return
	var spawn_pcount =  world.get_node("spawn_points").get_child_count()
	var spawn_pos = randi() % spawn_pcount
	emit_signal("client_log", "select spawn point(%s/%s)" % [spawn_pos, spawn_pcount])
...
#	player.flies = true # MUST CHANGE WHEN COLLISIONS ARE DONE
	...
	emit_signal("client_log", "==create player(%s) %s; name(%s)" % [id, players[id], players[id].data.username])
...
func loading_done(var error : int) -> void:
	if error == OK or error == ERR_FILE_EOF:
		emit_signal("client_log", "changing scene okay(%s)" % level_loader.error)
		emit_signal("loading_done")
	else:
		emit_signal("client_log", "error changing scene %s" % level_loader.error)
...
func _on_queue_attach_on_tree_change() -> void:
	...
	if get_tree():
		if _queue_attach_on_tree_change_prev_scene != str(get_tree().current_scene):
			...
			emit_signal("client_log", "qatc: Scene changed %s" % _queue_attach_on_tree_change_prev_scene)
			for p in _queue_attach:
				if _queue_attach[p].has("node"):
					emit_signal("client_log", "qatc: node %s(%s) permanent %s" % [p, _queue_attach[p].node, _queue_attach[p].permanent])
				if _queue_attach[p].has("signal"):
					emit_signal("client_log", "qatc: signal %s(%s) permanent %s" % [p, _queue_attach[p].signal, _queue_attach[p].permanent])
		else:
				...
				if obj:
					...
					if _queue_attach[p].has("signal"):
						...
						if not _queue_attach[p].permanent:
							emit_signal("client_log", "qatc, emit and remove: %s(%s) permanent %s" % [p, _queue_attach[p].signal, _queue_attach[p].permanent])
					...
					if not _queue_attach[p].permanent:
						emit_signal("client_log", "qatc, attached and removed: %s(%s) permanent %s" % [p, _queue_attach[p].node, _queue_attach[p].permanent])
						_queue_attach.erase(p)
...
func _on_server_user_connected(id : int) -> void:
	emit_signal("client_log", "user connected %s" % id)

func _on_server_user_disconnected(id : int) -> void:
	emit_signal("client_log", "user disconnected %s" % id)

func _on_server_tree_user_connected(id : int) -> void:
	emit_signal("client_log", "tree user connected %s" % id)

func _on_server_tree_user_disconnected(id : int) -> void:
	emit_signal("client_log", "tree user disconnected %s" % id)
	unregister_client(id)


func _on_player_scene() -> void:
	emit_signal("client_log", "scene is player ready, checking players(%s)" % players.size())
	if options.debug:
		for p in players:
			emit_signal("client_log", "player %s" % players[p])
	for p in players:
		create_player(p)
...
func _on_connection_failed() -> void:
	emit_signal("client_log", "client connection failed to %s(%s):%s" % [player_get("host"), player_get("ip"), player_get("port")])
...
func _on_connected_to_server() -> void:
	emit_signal("client_log", "client connected to %s(%s):%s" % [player_get("host"), player_get("ip"), player_get("port")])
	bind_signal("connection_failed", '', get_tree(), self, MODE.DISCONNECT)
	bind_signal("connected_to_server", '', get_tree(), self, MODE.DISCONNECT)
	RoleClient = true
...
func _on_client_log(msg) -> void:
	printd("client log: %s" % msg)
